...
function encodeAppArgs(args) {
  return args.map((arg) => {
    ...
    if (typeof arg === "bigint") return algosdk.encodeUint64(Number(arg));
    if (typeof arg === "number") return algosdk.encodeUint64(arg);
    return arg;
  });
}

function makeDepositTx(sender, receiver, assetId, amount, note, sp) {
  if (assetId === 0) {
...
export async function buildAddLiquidityTxns({
  ...
  if (poolState.totalLpTokens === 0n) {
    ...
    expectedLp = sqrt(product) - 1000n;
    if (expectedLp <= 0n) throw new Error("Initial liquidity too small");
  } else {
  ...
  const minimumLp = (expectedLp * (10000n - slippageBps)) / 10000n;
