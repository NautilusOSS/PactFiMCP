...
export async function getPools({ symbol, is_verified, pool_type, limit } = {}) {
  ...
  if (symbol) {
    const [d1, d2] = await Promise.all([
      fetchPools({ ...params, primary_asset__unit_name: symbol }),
      fetchPools({ ...params, secondary_asset__unit_name: symbol }),
    ]);
...
export async function getPool(appId) {
  const onChain = await getPoolStateOnChain(appId);
  const apiPool = await fetchPoolByAppId(appId, {
    primaryAssetId: onChain.primaryAssetId,
    secondaryAssetId: onChain.secondaryAssetId,
  });
